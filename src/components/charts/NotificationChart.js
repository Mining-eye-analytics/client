...
export const NotificationChart = () => {
  ...
  const chartData = {
    ...
    datasets: cctvList?.map((cctv, index) => {
      return {
        ...cctv,
        label: cctv.name + " - " + cctv.location,
        data: notificationData
          ?.filter((data) => data.cctv_id === cctv?.id)
          ?.map((data) => data.count),
        fill: false,
        borderColor: `rgb(${chartColor[index]})`,
        backgroundColor: `rgba(${chartColor[index]}, 0.5)`,
        tension: 0.1,
      };
    }),
  };
  ...
  useEffect(() => {
    ...
    customAxios({
      method: "GET",
      url:
        "/dashboards/notification_profile?" +
